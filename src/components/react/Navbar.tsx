...
export const Navbar = ({ currentPath = "" }: NavbarProps) => {
  ...
  // Fallback for client-side if currentPath is not provided (though it should be)
    ...
    >
      <div className="container mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <a href="/" className="flex items-center space-x-3 group shrink-0" aria-label="Aexaware Infotech Home">
            <img
              src={Logo.src}
              ...
              {navLinks.map((link) =>
                link.dropdown ? (
                  ...
                  <a
                    ...
                    href={link.path}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className={`text-base font-medium transition-colors hover:text-primary ${isActive(link.path)
                      ? "text-primary"
                      : "text-muted-foreground"
                      }`}
                  >
